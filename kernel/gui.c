...
enum { TERM_READY = 0, TERM_HELP, TERM_APPS, TERM_INFO, TERM_VER, TERM_UNKNOWN };
...
static void draw_terminal_result(void){
    switch(terminal_result){
        case TERM_HELP:
            text(22,9,"help  clear  cls  apps  info  ver",ATTR_NORMAL);
...
void gui_init(void){selected=0;active_app=-1;terminal_result=TERM_READY;terminal_reset_input();gui_draw();}
void gui_handle_key(char key){
    if(active_app<0){
        ...
        else if(key=='\r'||key=='\n'){active_app=(int8_t)selected;terminal_result=TERM_READY;terminal_reset_input();gui_draw();}
        return;
    }
